...
var sushi = Bodies.circle(400, 300, 50, {
    render: { sprite: { texture: './sushi.png' } },
    collisionFilter: {
        category: draggableCategory,
    },
});
var riceball= Bodies.circle(350, 300, 50, {
    render: { sprite: { texture: './riceball.png' } },
    collisionFilter: {
        category: draggableCategory,
    },
});
var santa = Bodies.circle(450, 300, 50, {
    render: { sprite: { texture: './hearteyes.png' } },
    collisionFilter: {
        category: draggableCategory,
    },
});
...
function finderGame() {
    sushi.velocity = {x: 2 * Math.random(), y: 2 * Math.random()};
    riceball.velocity = {x: 2 * Math.random(), y: 2 * Math.random()};
    santa.velocity = {x: 2 * Math.random(), y: 2 * Math.random()};
    World.remove(engine.world, [pinata, string, rock, elastic]);
    World.add(engine.world, [sushi, riceball, santa]);
    var i = 0;
    for (i = 0; i < 50; i++) {
...
Events.on(engine, 'collisionStart', function(event) {
    ...
    for (var i = 0; i < pairs.length; i++) {
        ...
        if (!isFinderGame && (pair.bodyA == pinata || pair.bodyB == pinata)) {
            ...
            pair.bodyA == sushi && pair.bodyB == magnifier ||
            pair.bodyA == magnifier && pair.bodyB == sushi) {
            showMessage("Sushi & Dumpling Class");
        } else if (
            pair.bodyA == riceball && pair.bodyB == magnifier ||
            pair.bodyA == magnifier && pair.bodyB == riceball) {
            showMessage("Sunday, January 3rd, 8pm");
        } else if (
